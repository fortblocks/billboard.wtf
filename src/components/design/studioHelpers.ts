...
export type { EmojiEntry, AnimatedSticker, PopularIcon } from "./emojiData";
export { EMOJI_LIB, searchEmojis, ANIMATED_STICKERS, searchAnimated, POPULAR_ICONS, searchPopularIcons } from "./emojiData";
...
export async function searchIconify(query: string, limit = 48): Promise<{ id: string; svgUrl: string }[]> {
  ...
  try {
    ...
    return icons.map((id) => {
      ...
      return {
        id,
        svgUrl: `https://api.iconify.design/${prefix}/${name}.svg?height=128`,
      };
    });
  } catch {
    return [];
    ...
    if (layer.type === "text") {
      ...
      if (layer.src) {
        try {
          ...
          withLayerTransform(ctx, lx, ly, lw, lh, layer, () => {
            drawFitted(ctx, img, lx, ly, lw, lh, "contain");
          });
        } catch { /* skip */ }
      } else if (layer.emoji) {
        withLayerTransform(ctx, lx, ly, lw, lh, layer, () => {
          ctx.font = `${lh * 0.85}px serif`;
