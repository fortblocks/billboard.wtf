...
import { FONTS, STICKERS, STICKER_PACKS, EMOJI_LIB, searchEmojis, searchIconify, ANIMATED_STICKERS, searchAnimated, uid, rasterizeFace } from "./studioHelpers";
        ...
        <div className="fixed bottom-8 left-1/2 z-50 w-[min(560px,94vw)] -translate-x-1/2 rounded-2xl border border-white/15 bg-neutral-900/98 p-3 shadow-2xl backdrop-blur">
          ...
          <div className="mb-2">
            <input
              ...
              placeholder={stickerTab === "emoji" ? "Search emoji — fire, smile, rocket, arm…" : stickerTab === "icons" ? "Search icons — rocket, heart, logo…" : "Search animated — fire, rocket, arm…"}
          ...
          <div className="max-h-[44vh] overflow-y-auto pr-1">
            {stickerTab === "emoji" && (
              stickerQuery.trim() ? (
                ...
                <div className="space-y-3">
                  ...
                  <p className="px-1 pt-1 text-[11px] text-white/35">{EMOJI_LIB.length}+ emoji searchable — try arm, fire, smile…</p>
                </div>
              )
            )}
            {stickerTab === "icons" && (
            ...
            {stickerTab === "gif" && (
              <div className="space-y-3">
                <div className="flex flex-wrap gap-1.5">
                  {searchAnimated(stickerQuery, 48).map((item) => (
                    <button
                      key={item.id}
                      type="button"
                      title={item.label}
                      onClick={() => addIconSticker(item.src, "gif")}
                      className="flex h-12 w-12 items-center justify-center rounded-xl bg-white/5 p-1.5 transition hover:scale-105 hover:bg-white/15"
                    >
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                ...
                {stickerQuery.trim() && searchAnimated(stickerQuery, 48).length === 0 && (
                  <p className="px-1 text-xs text-white/40">No matches — try fire, rocket, arm, heart…</p>
                )}
                <div className="flex items-center gap-2 border-t border-white/10 pt-3">
                  <p className="flex-1 text-[11px] text-white/40">{ANIMATED_STICKERS.length} stickers · upload your own GIF</p>
                  <button
                    type="button"
                    onClick={() => {
                      setSelectedId(null);
