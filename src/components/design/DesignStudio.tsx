...
export function DesignStudio({ entry }: { entry: BoardEntry }) {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "Escape") { if (inlineEditingId) { setInlineEditingId(null); return; } setSelectedId(null); setPlusOpen(false); setFontPanelOpen(false); setHelpOpen(false); return; }
  ...
  }, [selectedId, layers, undo, redo, duplicateLayer, removeLayer, updateLayer, inlineEditingId]);
    ...
    <div className="relative h-full w-full overflow-hidden bg-neutral-950">
      <div className="absolute inset-0 flex items-end justify-center">
        <div className="relative" style={{ width: BOARD_WIDTH, transform: `translateY(${BOARD_OFFSET_Y}px)` }}>
          ...
          <div ref={faceRef} className="absolute z-20 overflow-hidden bg-white" style={{ left: FACE.left, top: FACE.top, right: FACE.right, bottom: FACE.bottom }} onClick={() => { setSelectedId(null); setInlineEditingId(null); setPlusOpen(false); setFontPanelOpen(false); }} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerLeave={onPointerUp}>
            {layers.slice().filter((l) => l.visible !== false).sort((a, b) => a.z - b.z).map((layer) => {
              ...
              return (
                <div key={layer.id} className="absolute" style={{ left: `${layer.x}%`, top: `${layer.y}%`, width: `${layer.w}%`, height: `${layer.h}%`, zIndex: layer.z }} onClick={(e) => e.stopPropagation()} onDoubleClick={(e) => {
                        ...
                        <textarea
                          ...
                          onKeyDown={(e) => {
                            ...
                            if (e.key === "Enter" && !e.shiftKey) {
                              e.preventDefault();
                              updateLayer(layer.id, { text: (e.target as HTMLTextAreaElement).value });
                              setInlineEditingId(null);
                            }
                            e.stopPropagation();
                          }}
                          ...
                          className="h-full w-full resize-none bg-transparent px-[3%] py-[3%] leading-tight outline-none"
                          style={{
                  ...
                  {isSel && !layer.locked && !inlineEditingId && (
                    <>
      ...
      {selected && selected.id !== "__stickers__" && !inlineEditingId && (
