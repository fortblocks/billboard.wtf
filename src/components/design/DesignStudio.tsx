...
import { FONTS, STICKERS, STICKER_PACKS, uid, rasterizeFace } from "./studioHelpers";
...
export function DesignStudio({ entry }: { entry: BoardEntry }) {
  ...
  const addSticker = (emoji: string) => {
    const maxZ = layers.reduce((m, l) => Math.max(m, l.z), 0);
    const layer: StickerLayer = { id: uid(), type: "sticker", emoji, x: 38, y: 28, w: 24, h: 40, z: maxZ + 1, rotation: 0, opacity: 1 };
    setLayersTracked((p) => [...p, layer]);
    setSelectedId(layer.id);
    setPlusOpen(false);
  };
  ...
  const runAi = async () => {
    ...
    try {
      ...
      const layer: TextLayer = { id: uid(), type: "text", text: words, x: 8, y: 30, w: 84, h: 40, z: maxZ + 1, rotation: 0, opacity: 1, fontFamily: "var(--font-anton), Impact, sans-serif", fontSize: 5.5, color: "#0a0a0a", fontWeight: 400, fontStyle: "normal", align: "center" };
      setLayersTracked((p) => [...p, layer]);
      setSelectedId(layer.id);
      setAiOpen(false);
      setAiPrompt("");
  ...
  return (
    <div className="relative h-full w-full overflow-hidden bg-neutral-950">
      ...
      {selectedId === "__stickers__" && (
        <div className="fixed bottom-8 left-1/2 z-50 w-[min(520px,94vw)] -translate-x-1/2 rounded-2xl border border-white/15 bg-neutral-900/98 p-3 shadow-2xl backdrop-blur">
