...
export function DesignStudio({ entry }: { entry: BoardEntry }) {
  ...
  return (
    <div className="relative h-full w-full overflow-hidden bg-neutral-950">
      {/* Listing — fixed strip under site nav */}
      <div className="absolute left-0 right-0 top-0 z-40 flex items-center gap-2 border-b border-white/10 bg-neutral-950/95 px-3 py-2.5 backdrop-blur-md sm:px-4">
        <input
          value={brand}
          onChange={(e) => setBrand(e.target.value)}
          className="h-9 min-w-0 flex-[1.2] rounded-lg border border-white/12 bg-white/[0.06] px-3 text-sm text-white placeholder:text-white/30 outline-none focus:border-white/30"
          placeholder="Brand name"
        />
        <input
          value={url}
          onChange={(e) => setUrl(e.target.value)}
          className="h-9 min-w-0 flex-1 rounded-lg border border-white/12 bg-white/[0.06] px-3 text-sm text-white placeholder:text-white/30 outline-none focus:border-white/30"
          placeholder="https://"
        />
        <input
          value={xHandle}
          onChange={(e) => setXHandle(e.target.value)}
          className="h-9 w-[7.5rem] shrink-0 rounded-lg border border-white/12 bg-white/[0.06] px-3 text-sm text-white placeholder:text-white/30 outline-none focus:border-white/30"
          placeholder="@handle"
        />
        <button
          type="button"
          onClick={shareOnX}
          className="h-9 shrink-0 rounded-full border border-white/15 px-3.5 text-[11px] text-white/70 transition hover:border-white/35 hover:text-white"
        >
          Share on X
        </button>
        <button
        ...
        <button
          ...
          onClick={lock}
          disabled={!brand.trim()}
          className="h-9 shrink-0 rounded-full bg-white px-5 text-[11px] font-semibold text-neutral-900 transition hover:bg-neutral-200 disabled:opacity-40"
        >
          Lock & go live
        </button>
      </div>

      {/* Board stack — identical to LiveBoard (frame in-flow, face absolute with FACE) */}
      <div className="absolute inset-x-0 bottom-0 top-14 flex items-end justify-center sm:top-16">
        <div
          className="relative"
          style={{
            width: BOARD_WIDTH,
