...
export function DesignStudio({ entry }: { entry: BoardEntry }) {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "Escape") { setSelectedId(null); setPlusOpen(false); setFontPanelOpen(false); setHelpOpen(false); return; }
  ...
  }, [selectedId, layers, undo, redo, duplicateLayer, removeLayer, updateLayer]);
  ...
  const addText = () => {
    ...
    const layer: TextLayer = { id: uid(), type: "text", text: brand.trim() || "YOUR TEXT", x: 10, y: 28, w: 80, h: 36, z: maxZ + 1, rotation: 0, opacity: 1, fontFamily: "var(--font-anton), Impact, sans-serif", fontSize: 7, color: "#0a0a0a", fontWeight: 400, fontStyle: "normal", align: "center" };
  ...
  const addSticker = (emoji: string) => {
    const maxZ = layers.reduce((m, l) => Math.max(m, l.z), 0);
    const layer: StickerLayer = { id: uid(), type: "sticker", emoji, x: 40, y: 30, w: 20, h: 35, z: maxZ + 1, rotation: 0, opacity: 1 };
    setLayersTracked((p) => [...p, layer]);
    setSelectedId(layer.id);
    setPlusOpen(false);
  };
  ...
  const onUpload = (file: File) => {
    ...
    reader.onload = () => {
      const maxZ = layers.reduce((m, l) => Math.max(m, l.z), 0);
      const layer: ImageLayer = { id: uid(), type: "image", src: String(reader.result), x: 10, y: 5, w: 80, h: 90, z: maxZ + 1, rotation: 0, opacity: 1 };
      setLayersTracked((p) => [...p, layer]);
      setSelectedId(layer.id);
      setPlusOpen(false);
    };
    ...
    <div className="relative h-full w-full overflow-hidden bg-neutral-950">
      <div className="absolute inset-0 flex items-end justify-center">
        <div className="relative" style={{ width: BOARD_WIDTH, transform: `translateY(${BOARD_OFFSET_Y}px)` }}>
          ...
          {helpOpen && (
            <div className="absolute z-40 rounded-lg border border-neutral-200 bg-white p-4 text-[13px] leading-relaxed text-neutral-700 shadow-xl" style={{ top: "23%", left: "7.7%", width: "min(340px, 84%)" }} onClick={(e) => e.stopPropagation()}>
              ...
              <ol className="list-decimal space-y-1.5 pl-4">
                <li>Tap <strong>+</strong> to add text, image, or sticker.</li>
                <li>Drag to move. Corner handles to resize (Shift = lock ratio).</li>
                <li>Violet knob above selection rotates — snaps near 45°.</li>
          ...
          <div ref={faceRef} className="absolute z-20 overflow-hidden bg-white" style={{ left: FACE.left, top: FACE.top, right: FACE.right, bottom: FACE.bottom }} onClick={() => { setSelectedId(null); setPlusOpen(false); setFontPanelOpen(false); }} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerLeave={onPointerUp}>
            {layers.slice().filter((l) => l.visible !== false).sort((a, b) => a.z - b.z).map((layer) => {
              ...
              return (
                <div key={layer.id} className="absolute" style={{ left: `${layer.x}%`, top: `${layer.y}%`, width: `${layer.w}%`, height: `${layer.h}%`, zIndex: layer.z }} onClick={(e) => e.stopPropagation()}>
                  <div className={`h-full w-full ${isSel ? "outline outline-2 outline-sky-400 outline-offset-[-1px]" : ""}`} style={{ transform: contentTransform || undefined, opacity: layer.opacity ?? 1, cursor: layer.locked ? "default" : dragging && isSel ? "grabbing" : "grab" }} onPointerDown={(e) => onPointerDown(e, layer.id)}>
                    {layer.type === "text" && (
                    ...
                    {layer.type === "image" && (/* eslint-disable-next-line @next/next/no-img-element */ <img src={layer.src} alt="" className="h-full w-full object-cover" draggable={false} style={{ pointerEvents: "none" }} />)}
                    {layer.type === "sticker" && (<div className="flex h-full w-full items-center justify-center" style={{ fontSize: "min(8vw, 12vh)", pointerEvents: "none" }}>{layer.emoji}</div>)}
                  </div>
                  {isSel && !layer.locked && (
                    <>
      ...
      {selected && selected.id !== "__stickers__" && (
        <div className="fixed bottom-8 left-1/2 z-50 flex max-w-[96vw] -translate-x-1/2 flex-wrap items-center gap-1 rounded-2xl border border-white/15 bg-neutral-900/95 px-3 py-2.5 shadow-2xl backdrop-blur">
          {selected.type === "text" && (
            <>
              <input value={selected.text} onChange={(e) => updateLayer(selected.id, { text: e.target.value })} className="min-w-[120px] rounded-lg border border-white/15 bg-black/40 px-2.5 py-1.5 text-sm text-white outline-none" />
      ...
      {selectedId === "__stickers__" && (
        <div className="fixed bottom-8 left-1/2 z-50 flex max-w-[95vw] -translate-x-1/2 flex-wrap items-center gap-1 rounded-2xl border border-white/15 bg-neutral-900/95 px-4 py-3 shadow-2xl backdrop-blur">
          {STICKERS.map((e) => (<button key={e} type="button" onClick={() => addSticker(e)} className="rounded-lg bg-white/5 px-2 py-1 text-xl hover:bg-white/15">{e}</button>))}
      ...
      <input ref={fileRef} type="file" accept="image/jpeg,image/png,image/webp,image/gif" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) onUpload(f); }} />
    </div>
  );
}
